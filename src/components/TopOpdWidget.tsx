...
/** Widget ringkas Top OPD di beranda — sumber: GET /api/analytics (opdBreakdown).
...
export default function TopOpdWidget() {
  ...
  React.useEffect(() => {
    // Pola anti setState-sinkron-di-effect (lihat f2e364f): defer via setTimeout.
  ...
  if (error) {
    return (
      <div className="bg-[var(--surface-card)] border border-[var(--border)] rounded-2xl p-5">
        <h2 className="text-sm font-bold text-[var(--text)] mb-3">🏛️ Top OPD</h2>
  ...
  return (
    <div className="bg-[var(--surface-card)] border border-[var(--border)] rounded-2xl p-5">
      <div className="flex items-baseline justify-between mb-4">
        <h2 className="text-sm font-bold text-[var(--text)]">🏛️ Top {TOP_N} OPD — Jumlah Indikator</h2>
        <Link
